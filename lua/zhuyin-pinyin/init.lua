...
function M.transform_selection(func)
	...
	local lines = vim.api.nvim_buf_get_text(0, start_row, start_col - 1, end_row - 1, end_col - 1, {})
	local text = table.concat(lines, "\n")
	local transformed, err = func(text)
	if not transformed then
		return nil, err
	end
	vim.api.nvim_buf_set_text(0, start_row, start_col - 1, end_row - 1, end_col - 1, { transformed })
	return text
end
